...
runProgram(async () => {
    ...
    program.addOption(new Option(`-t --type <string>`, `Network type`).choices([`evm`, `substrate`]))
    program.addOption(new Option(`-r --release <string>`, `Release name`).choices([`FireSquid`, `ArrowSquid`]))
...
function printEvmArchives(filter: Filter) {
    ...
    for (let archive of archivesRegistryEVM().archives) {
        ...
        for (let provider of archive.providers) {
            if (filter.release != null && filter.release != provider.release) continue
